    ...
    def train_pred_prober(self):
        ...
        for epoch in tqdm(range(epochs), desc=f"Probe prediction epochs"):
            for batch in tqdm(dataset, desc="Probe prediction step"):
                ...
                # Forward pass using the `forward_inference` method of JEPAModel
                pred_encs = model.forward_inference(actions=actions, states=init_states)
                pred_encs = pred_encs.transpose(0, 1)  # # BS, T, D --> T, BS, D
                pred_encs = pred_encs.to(self.device)
